...
FONT_SIZE = (VAR_SIZE-SIZE) // MAX_OBJECT
...
class SampleObject:
    ...
    def draw_board(self):
        ...
        start_rect = pygame.Rect(SIZE, 0, HOR_SIZE-SIZE, SIZE)
            ...
            for i in range(self.obj_num):
                if self.is_obj(x,y,i):
                    ...
                    if self.obj[i].pos_x == x and self.obj[i].pos_y == y:
                        ...
                        self.panel[x][y].has_building = False
                    self.click[x][y] = RED
                    return True
    ...
    def put_obj(self, x, y):
        ...
        if can_put:
            ...
            print(self.select_obj_num)
            self.panel[x][y].has_building = True
            self.select_obj_num = -1
            return True
        return False
    ...
    def get_space(self):
        text_num = 0
        text_rect = pygame.Rect(0, SIZE, HOR_SIZE, VAR_SIZE-SIZE)
        pygame.draw.rect(screen, BLACK, text_rect)
        for x in range(BORAD_SIZE):
            for y in range(BORAD_SIZE):
                if self.panel[x][y].has_building == True:
                    text = "building_pos=" + str(x) + "," + str(y)
                    print(text)
